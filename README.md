...
**Playing ≠ HDR, it turns out.** Once the animation itself worked, RTX Video HDR stopped engaging on the converted GIF — even though a frozen single-frame conversion (the pre-fix, non-animated behavior) reportedly did get HDR applied. So it's not simply "canvas-stream video never gets RTX HDR"; something about content actually changing frame-to-frame specifically breaks it. Current best guess, unconfirmed: RTX may need a moment after playback starts to detect and lock onto the video, and content changing from frame 1 onward could be interrupting that. As a cheap experiment, a converted GIF now holds on its first frame for 1.5s (`HDR_SETTLE_MS` in `content.js`) before it's allowed to actually start animating, on the theory that matching the exact "stays still" case that worked gives RTX the same window to engage first. Unverified — this is a guess, not a diagnosed fix.
